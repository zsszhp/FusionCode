...
from fusioncode.enhancement.inpainting import inpaint_missing
...
from fusioncode.decoding.halcon_decoder import decode_halcon
...
class Orchestrator:
    ...
    def _decode_with_engines(self, img):
        ...
        if self.config["engines"]["zbar"]["enable"]:
            results.extend(decode_zbar(img))

        if self.config["engines"]["opencv_qr"]["enable"]:
            results.extend(decode_opencv_qr(img))

        if self.config["engines"]["zxing"]["enable"]:
            results.extend(decode_zxing(img))

        if self.config["engines"]["commercial"]["enable"]:
            results.extend(decode_commercial(img))

        if self.config["engines"]["halcon"]["enable"]:
            results.extend(decode_halcon(img))
